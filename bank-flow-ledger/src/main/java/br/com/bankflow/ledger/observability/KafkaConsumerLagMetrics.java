...
public class KafkaConsumerLagMetrics implements DisposableBean {
	...
	public KafkaConsumerLagMetrics(
			...
			@Value("${bank-flow.kafka.topics.ledger-reversals}") String ledgerReversalsTopic
	) {
		this.groupId = groupId;
		this.topics = List.of(accountCreatedTopic, ledgerMovementsTopic, ledgerReversalsTopic);
		Properties properties = new Properties();
		properties.put("bootstrap.servers", bootstrapServers);
		this.adminClient = AdminClient.create(properties);
		Gauge.builder("kafka_consumer_lag", lag, AtomicLong::get)
				.description("Total Kafka consumer lag")
				.tag("service", "bank-flow-ledger")
