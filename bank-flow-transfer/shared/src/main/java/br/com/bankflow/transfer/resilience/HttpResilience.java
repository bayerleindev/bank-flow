package br.com.bankflow.transfer.resilience;
...
public class HttpResilience {
	...
	public HttpResilience(
			...
			@Value("${bank-flow.resilience.http.retry.backoff-ms:100}") long backoffMs,
			...
			@Value("${bank-flow.resilience.http.circuit-breaker.sliding-window-size:20}") int slidingWindowSize,
			@Value("${bank-flow.resilience.http.circuit-breaker.minimum-calls:10}") int minimumCalls,
			@Value("${bank-flow.resilience.http.circuit-breaker.open-duration-ms:10000}") long openDurationMs
	) {
