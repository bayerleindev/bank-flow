...
import br.com.bankflow.balance.observability.BusinessCorrelation;
...
public class LedgerPostingProjectionService {
	...
	public LedgerPostingProjectionResult project(LedgerPostingCreatedEvent event) {
		try (BusinessCorrelation.Scope ignored = BusinessCorrelation.ledgerPosting(event.externalId(), event.entryId())) {
			...
			try {
				...
				balanceMetrics.recordProjection(
						sample,
						LedgerPostingProjectionResult.PROJECTED,
						event.lines().size(),
						event.entryType(),
